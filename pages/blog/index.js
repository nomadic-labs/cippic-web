...
export const getStaticProps = async ({ params }) => {
    ...
    const blogPostsRes = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_DOMAIN}/api/articles?${blogArticlesQuery}`)
    const blogPostsJson = await blogPostsRes.json()
    const blogPosts = blogPostsJson.data

    const contactRes = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_DOMAIN}/api/organization-information?${contactQuery}`)
    const contact = await contactRes.json()

    const content = { blogPosts, contact: { ...contact.data.attributes } }
